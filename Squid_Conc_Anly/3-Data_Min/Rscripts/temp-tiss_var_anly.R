    ...
    for(i in 1:8) {
      ...
      if(year[h]=='2019'){
        if(unique(plotting_dataset$vars)=='Latitude'){
          ...
          name0 <- paste(plts,"_", variable, year[h], sep = "")
          list0names <- append(list0names,name0)
        }else{
          #print(str(plotting_dataset))
          plt <-plotting_dataset %>% ggplot(aes(Values, concentrations, colour = Tissue, group=Tissue)) +
            geom_smooth(method=lm, se=FALSE)+
            labs(title = paste(tools::toTitleCase(year[h]),variable1,"Vs Concentrations mg/kg",sep =" "),
        ...
        if(unique(plotting_dataset$vars)=='Latitude'){
          ...
          name1 <- paste(plts,"_", variable, year[h], sep = "")
        ...
        if(unique(plotting_dataset$vars)=='Latitude'){
          ...
          name2 <- paste(plts,"_", variable, year[h], sep = "")
...
save_graphs <- function(graph_list) {
  ...
  for (year in years) {
    ...
    # Define the folder where you want to save the PNG files for each tissue
      output_folder <- file.path("/Users/mrnobody/Documents/GitHub/Squid_Fest/Squid_Conc_Anly/3-Data_Min/Outputs/temp-tiss_var_anly")
