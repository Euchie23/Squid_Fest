...
pca_kmeans_cluster_explorer <- function(data_list, remove.vars = FALSE) {
  ...
  output_folder <- file.path("Squid_Conc_Anly/3-Data_Min/Outputs/pca_kmns_clstr/")
  ...
  for (tissue in tissues) {
    ...
    kmeans_biplot_all <- fviz_pca_ind(pca_all, geom.ind = "point", pointshape = 21,
                                      ...
                                      palette = "jco",
                                      title = paste("K-means Clusters -", tissue, "- All Yrs"))
    ...
    # Save PNG
    png(file.path(tissue_folder, paste0(plts, '_', tissue,"_All_Yrs_PCA_KMns_Bxplt.png")), width = 1800, height = 1200)
    ...
    for (year in years) {
      ...
      png(file.path(tissue_folder, paste0(plts, '_', tissue, "_", year, "_PCA_KMns_Bxplt.png")), width = 1800, height = 1200)
