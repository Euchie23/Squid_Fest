...
process_dataset<- function(data, keep_LOQ_values=FALSE) {
  ...
  if (grepl("Metal", colnames(data)[16])) {
    ...
    data <- data %>%
      relocate(1:15, colnames(data)[16:19]) %>%
      mutate(across(4, ~ paste0(., ".")))  # Add "." to column 4
...
datasets_for_trace_metals <- process_dataset(tracemetals_data, keep_LOQ_values = TRUE) 
...
datasets_for_organic_compounds <- process_dataset(organiccompounds_data, keep_LOQ_values = TRUE)
...
comparing_sizes_per_pollutant <- function (data_list, variable, remove.zeroes = FALSE){
  ...
  print(dataset_with_numerical_values)
   ...
   pollutant <- colnames(dataset_with_numerical_values[,c(number_range)])[h]
    ...
    if (nrow(long_dataset) != 0) {
      ...
      #Plotting graphs per pollutant
        ...
        name0 <- paste(pollutant,'_',var_abb, sep = "")
...
size_comparison_results_per_pollutant <- comparing_sizes_per_pollutant (datasets_for_trace_metals, variable='dta_km', remove.zeroes = FALSE) 
...
output_folder <- file.path("/Users/mrnobody/Documents/GitHub/Squid_Fest/Squid_Conc_Anly/3-Data_Min/Outputs/size_comprsn_per_polltnt")
