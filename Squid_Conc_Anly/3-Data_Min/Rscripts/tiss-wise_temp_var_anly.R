...
comparing_years_per_tissue_per_variable <- function (data_list, remove.zeroes = FALSE){
  ...
  for (h in 1:4) {
    ...
    for(i in 1:8) {
      cat("\n")
      #saved for naming graph and x axis titles
      variable <- colnames(subsetted_dataset_with_numerical_values)[i]
      
      #saved for keeping full name for naming graph and x axis title for specific graphs with long names.. their were showrten from variable so they can be saved with shorter names resulting in shorter file paths
      variable1 <- colnames(subsetted_dataset_with_numerical_values)[i]
      print(variable)
    ...
    if (remove.zeroes == FALSE) {
      ...
      name_map <- c(
        "Month_of_Capture" = "Cap_Mnth",
        "Mantle_length_mm" = "Mant_Len_mm",
        "Maturity_level"   = "Mat_Lev",
        "Wet_Weight_g"     = "Wet_Wght_g"
      )
      
      
      # Get the mapped value (abbreviated_vars)
      if (variable %in% names(name_map)) {
      ...
      if(tissues[h]=='liver'){
        if(unique(plotting_dataset$vars)=='Latitude'){
          ...
          name0 <- paste(plts, '_', variable,'_', tissues[h], sep = "")
          list0names <- append(list0names,name0)
        }else{
          #print(str(plotting_dataset))
          plt <-plotting_dataset %>% ggplot(aes(Values, concentrations, colour = Year, group=Year)) +
            geom_smooth(method=lm, se=FALSE)+
            labs(title = paste(tools::toTitleCase(tissues[h]),variable1,"Vs Concentrations mg/kg",sep =" "),
                 y = "Concentration mg/kg", x = paste(variable1))+
            facet_wrap(vars(pollutants), labeller = as_labeller(icons_markdown), scales ="free", ncol=3, drop = TRUE) +
            ggh4x::facetted_pos_scales(
              x = scales_x,
              y = scales_y
        ...
        if(unique(plotting_dataset$vars)=='Latitude'){
          ...
          name1 <- paste(plts, '_', variable,'_', tissues[h], sep = "")
        ...
        if(unique(plotting_dataset$vars)=='Latitude'){
          ...
          name2 <- paste(plts, '_', variable,'_', tissues[h], sep = "")
        ...
        if(unique(plotting_dataset$vars)=='Latitude'){
          ...
          name3 <- paste(plts, '_', variable,'_', tissues[h], sep = "")
...
save_graphs <- function(graph_list) {
  ...
  for (tissue_name in tissues) {
    ...
    # Define the folder where you want to save the PNG files for each tissue
      output_folder <- file.path("/Users/mrnobody/Documents/GitHub/Squid_Fest/Squid_Conc_Anly/3-Data_Min/Outputs/tiss-wise_temp_var_anly")
