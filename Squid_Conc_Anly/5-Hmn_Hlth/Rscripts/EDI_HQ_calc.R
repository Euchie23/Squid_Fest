...
EDI_and_HQ_calculations <- function(data_list, remove.zeroes = FALSE) {
  ...
  for (year in years) {
   ...
   # Reshaping the dataset from wide to long format for easier comparison across countries and percentiles
    ...
    name1 <- paste(year,"_HQbrplts", sep = "")
    ...
    name3 <- paste(year,"_EDI_HQ_stats", sep = "")
...
save_custom_outputs <- function(output_list) {
  ...
  for (year in years) {
    ...
    if (any(grepl("Metal", pollutant_types, ignore.case = TRUE))) {
      root_folder <- "Squid_Conc_Anly/5-Hmn_Hlth/EDI_HQ_plts_tbls"
      file_name <- print(paste("EDI_HQ_tbls_Met_",year,".xlsm", sep = ""))
      excel_file <- file.path(root_folder, file_name)
      plltnt <- "Met"
    } else {
      root_folder <- "Squid_Conc_Anly/5-Hmn_Hlth/EDI_HQ_plts_tbls"
      file_name <- print(paste("EDI_HQ_tbls_Org_",year,".xlsm", sep = ""))
      excel_file <- file.path(root_folder, file_name)
      plltnt <- "Org"
    }
    
    # Create year-specific folder
    output_folder <- file.path(root_folder)
    ...
    plot_filename <- paste0("EDI_HQ_plts_", year,'_',plltnt, ".png")
