...
colnames(x)[c(1,3)] <- c("ID", "DW")
   ...
   Final_res_OC = "1-Data_Prep/Data/Clean/Fnl_Ores_mgkg.csv"
...
files <- list.files(path = "1-Data_Prep/Data/Organics_Raw/2021_OC/")
path <- "1-Data_Prep/Data/Organics_Raw/2021_OC/"

dry_weight <- read.csv("1-Data_Prep/Data/Organic_Raw/Dry_wght /21drywght.csv", header= TRUE)

#distance to Argentina and Falkland Islands.distance to land (dtl)
distance_to_land <- read.csv("1-Data_Prep/Data/Squid_Catch/Dist_to_land.csv", header= TRUE)

#SQUID CATCH DATA...CHANGE BASED ON THE YEAR YOU ARE PROCESSING, Basic information = bi
squid_info <- read.csv("1-Data_Prep/Data/Squid_Catch/21_catch.csv", header= TRUE)
