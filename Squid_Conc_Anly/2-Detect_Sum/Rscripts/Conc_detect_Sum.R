...
tracemetals_data <- read.csv("Squid_Conc_Anly/1-Data_Prep/Fnl_Res_For_Anly/Fnl_Mres_mgkg.csv", header = TRUE)
...
organiccompounds_data <- read.csv("Squid_Conc_Anly/1-Data_Prep/Fnl_Res_For_Anly/Fnl_Ores_mgkg.csv", header = TRUE)
...
visualization <- function(data_list1){
  ...
  if (grepl("Metal", final_dataset_for_plotting_by_pollutants[1,'pollutant'])) {
    ...
    print(plot_pollutants)
    list0<-append(list(plot_pollutants),list0, 0)
    name0 <- paste("Conc_Detect_Sum_of_Polltnts", sep = "")
    list0names <- append(list0names,name0)
    ...
    print(plot_tissues)
    list0<-append(list(plot_tissues),list0, 0)
    name0 <- paste("Conc_Detect_Sum_of_Tiss", sep = "")
    list0names <- append(list0names,name0)
  }  
...
save_detection_summary_outputs <- function(detection_summary_list) {
  ...
  if (any(grepl("Metal", pollutant_types))) {
    output_folder <- "Squid_Conc_Anly/2-Detect_Sum/Outputs/Metals"
    excel_file <- file.path(output_folder, "Detect_Sum_Cnt_M.xlsm")
  } else {
    output_folder <- "Squid_Conc_Anly/2-Detect_Sum/Outputs/Organics"
    excel_file <- file.path(output_folder, "Detect_Sum_Cnt_O.xlsm")
  }
